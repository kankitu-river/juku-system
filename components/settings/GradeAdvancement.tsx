...
export function GradeAdvancement() {
  ...
  return (
    <div>
      ...
      {result && !result.error && (
        <div className="mb-4 flex items-center gap-2 bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-900 rounded-lg px-4 py-3 text-sm text-green-800 dark:text-green-200">
          ...
          {(result.skipped ?? 0) > 0 && (
            <span className="text-green-600 dark:text-green-300">（高3 {result.skipped}名はスキップ）</span>
          )}
        </div>
      )}

      {result?.error && (
        <div className="mb-4 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-4 py-3 text-sm text-red-700 dark:text-red-300">
          {result.error}
        </div>
      )}
        ...
        <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={isPending}
              className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 disabled:opacity-50 transition-colors"
            >
              キャンセル
            </button>
          </div>
