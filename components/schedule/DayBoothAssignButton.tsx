...
import { autoAssignBooths, autoAssignIntensivePeriod } from '@/app/(dashboard)/booths/actions'
...
export function DayBoothAssignButton({ dateStr, dow, termType }: Props) {
  ...
  function assignDay() {
    ...
    startTransition(async () => {
      ...
      setResult(res.error ? `エラー: ${res.error}` : `${res.assigned}件を割り当てました`)
      setTimeout(() => { setResult(null); router.refresh() }, 2500)
    })
  }
  ...
  function assignPeriod() {
    ...
    setResult(null)
    startTransition(async () => {
      const res = await autoAssignIntensivePeriod(dateStr)
      setResult(res.error ? `エラー: ${res.error}` : `${res.days}日分・${res.assigned}件を割り当てました`)
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      >
        ...
        {pending ? '割り当て中…' : 'この日のブース割り当て'}
      </button>
      {termType === 'intensive' && (
        <button
          onClick={assignPeriod}
          disabled={pending}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-navy text-white rounded-lg hover:bg-navy-light transition-colors disabled:opacity-50"
        >
          講習期間まとめて割り当て
        </button>
      )}
