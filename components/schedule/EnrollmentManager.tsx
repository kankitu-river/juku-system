      ...
      {!isFull && (
        <div>
          ...
          <input
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]"
          />
          {candidates.length === 0 ? (
              ...
              {candidates.map((student) => (
                <li key={student.id}>
                  <button
                    ...
                    disabled={isPending}
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg border border-gray-100 hover:bg-gray-50 hover:border-[#1E3A5F] transition-colors text-left"
                  >
                    <div>
                      <span className="text-sm font-medium text-gray-800">{student.name}</span>
                      <span className="text-xs text-gray-400 ml-2">{getDisplayGrade(student.grade)}</span>
                    </div>
                    <span className="text-xs text-[#1E3A5F] font-medium">+ 追加</span>
                  </button>
                </li>
              ))}
            </ul>
