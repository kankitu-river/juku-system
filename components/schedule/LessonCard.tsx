    ...
    return (
      ...
      >
        ...
        <span className="truncate text-[10px] opacity-80">
          {displayStudents.map((s) => `${s!.name}（${subject}）`).join(' ')}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1">
          {lesson.lesson_kind === 'temporary' && (
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-orange-400 text-white">臨時</span>
          )}
          {lesson.is_ps1 && (
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500 text-white">1対1</span>
          )}
        </div>
        <span className={[
      ...
      {/* メモ */}
      {lesson.notes && (
        <div className="flex items-start gap-1 text-[11px] opacity-70 border-t border-current border-opacity-20 pt-1.5">
